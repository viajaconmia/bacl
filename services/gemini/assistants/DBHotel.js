...
class DBHotel extends Assistant {
  ...
  async call(task, history, stack) {
    try {
      ...
      const params = Array.isArray(args.items) ? args.items[0] : (args.items || {});
...
const routeToAssistantFunctionDeclaration = {
  ...
  description:
    "Look for hotels matches in local db data.",
  parameters: {
 type: Type.ARRAY,
    items: {
      type: Type.OBJECT,
      properties: {
        ...
        p_tipo_hospedaje: { type: Type.STRING, description: "Tipo de hospedaje" },
        ...
        p_incluye_desayuno: { type: Type.STRING, description: 'Incluye desayuno: "SI" / "NO"' },
        ...
        p_transportacion: { type: Type.STRING, description: "Transportación: 'SI'/'NO'" },
      },
      required: [],
    },
  },
