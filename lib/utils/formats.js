...
class Formato {
  ...
  static precio(precio) {
    if ((typeof precio == "string" && isNaN(precio)) || precio === "")
  ...
  static uuid(id, prefijo) {
    ...
    if (typeof id !== "string")
      throw new Error(`Revisar el tipo del uuid con prefijo: ${prefijo}`);
    if (!(id.length >= 36 && id.length <= 40))
      throw new Error("Reviar la longitud del id");
    return id;
  }
}
