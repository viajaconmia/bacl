...
const impersonateUser = async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ error: "Falta el correo del usuario a impersonar" });
    }
    ...
    const { data, error } = await supabase.auth.admin.generateLink({
      ...
      options: {
        redirectTo:
          process.env.REDIRECT_URL || "https://viajaconmia.com/impersonado",
      },
    });
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Error al generar el link de impersonación" });
    }
