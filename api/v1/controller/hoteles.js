...
const AgregarHotel = async (req, res) => {
  ...
  try {
    ...
    let otros_impuestos_porcentaje2 =
      otros_impuestos_porcentaje == null ? 0 : otros_impuestos_porcentaje;
      ...
      return preferenciales.map((tarifa) => {
        ...
        const tarifaPreferencial = {
          ...
          sencilla: {
            ...
            incluye:
              tarifa.sencilla?.incluye !== undefined
          ...
          doble: {
            ...
            incluye:
              tarifa.doble?.incluye !== undefined
    ...
    if (!nombre || !direccion || !estado || !ciudad_zona || !tipo_negociacion) {
      ...
      return res.status(400).json({
        ...
        message:
          "Faltan campos obligatorios: nombre, id_cadena, direccion, estado, ciudad_zona,tipo_negociacion, vigencia_convenio",
      });
    }
...
const consultaHoteles = async (req, res) => {
  ...
  try {
    ...
    if (!hoteles) {
      res
        .status(404)
        .json({ message: "No se encontraron hoteles registrados" });
    } else {
      res
        .status(200)
        .json({ message: "Hoteles recuperados con exito", data: hoteles });
    }
  } catch (error) {
...
const actualizaHotel = async (req, res) => {
  ...
  let score_sistemas2 = score_sistemas == null ? 0 : score_sistemas;
  try {
    const hotel_actualizado = await executeSP(
      "sp_actualizar_hotel2",
      [
        id_hotel,
        id_cadena,
        nombre,
...
const filtra_hoteles = async (req, res) => {
  ...
  if (opc == 1) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error interno del servidor", errorinfo: error });
    }
  } else if (opc == 2) {
    try {
...
const BuscaHotelesPorTermino = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "No se encontraron hoteles para esa busqueda" });
  }
};
  ...
  try {
    ...
    const hotels = result.map((row) => ({
      ...
      hotel_info: {
        ...
        tarifas_hotel: result
          .filter((item) => item.id_hotel === row.id_hotel)
          .map((tarifa) => ({
    ...
    res
      .status(200)
      .json({ message: "Información recuperada correctamente", hotels });
  } catch (error) {
    console.error("Error al recuperar la información:", error);
    res
      .status(500)
      .json({ message: "Error al recuperar la información del hotel" });
  }
};
...
const readHotelesWithTarifaClient = async (req, res) => {
  try {
    const hoteles = await model.getHotelesWithTarifasClient();
    res.status(200).json({ data: hoteles });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: "Error en el servidor", details: error });
...
const eliminarLogicaTarifa = async (req, res) => {
  try {
    ...
    const { id_tarifa_preferencial_sencilla, id_tarifa_preferencial_doble } =
      req.body;
...
module.exports = {
  ...
  actualizaHotel,
  eliminaHotelLogico,
  consultaPrecioSencilla,
  consultaPrecioDoble,
  filtra_hoteles,
  getTarifasByIdHotel,
  paginacion,
  BuscaHotelesPorTermino,
  get_hotel_tarifas_by_nombre,
